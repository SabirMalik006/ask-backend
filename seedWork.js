...
const webProjects = projectsData.filter(p => p.category === 'Web Development');
...
const runSeed = async () => {
  try {
    ...
    console.log(`Found ${webProjects.length} Web Development projects in source list.\n`);
    ...
    for (let i = 0; i < webProjects.length; i++) {
      ...
      console.log(`[${i+1}/${webProjects.length}] Checking project: "${cleanedTitle}"...`);
      
      // Idempotency Check: search by title and category
      const existingDoc = await Work.findOne({ title: cleanedTitle, category: 'Web Development' });
      if (existingDoc) {
      ...
      const folderPath = await getProjectFolder('Web Development', cleanedClient || cleanedTitle, newId);
      ...
      const workDoc = {
        ...
        description: `Experience ASK Websolutions premium Web Development services customized for ${cleanedClient}. Built with state-of-the-art tech.`,
        heroImage,
        galleryImages,
        order: i
      };
    ...
    console.log(`\n=== SEEDING SUMMARY ===`);
    console.log(`Total Web Development Projects Processed: ${webProjects.length}`);
